...
static char *usb_device_descriptions[ NUM_SUPPORTED_USB_DEVICES ][ 2 ] = {
    { "Canon", "CanoScan N1220U" },  { "Canon", "CanoScan D660U" },          { "Canon", "CanoScan N650U" },
    { "Canon", "CanoScan LiDE 20" }, { "Canon", "CanoScan LiDE 25" },        { "Canon", "CanoScan LiDE 30" },
    { "Epson", "Perfection 1260" },  { "Hewlett-Packard", "ScanJet 2200c" }, { "Genius (KYE)", "ColorPage-HR6 V2" } };
...
int plustek_match_libusb_scanner( libusb_device_t *device ) {
    ...
    for ( index = 0; index < NUM_SUPPORTED_USB_DEVICES; index++ ) {
        if ( supported_usb_devices[ index ][ 0 ] == device->vendorID && supported_usb_devices[ index ][ 1 ] == device->productID ) {
            break;
        }
    }
...
const char *scanbtnd_get_backend_name( void ) { return backend_name; }
...
const scanner_t *scanbtnd_get_supported_devices( void ) { return plustek_scanners; }
...
int scanbtnd_get_button( scanner_t *scanner ) {
    ...
    */
    ...
    // fprintf( stderr, "button byte: 0x%02X\n", *bytes );
    if ( !*bytes ) { // 0 in case not yet initialized
        syslog( LOG_INFO, "not initialized (read returns 0)" );
        return -1; // trigger sane-find-scanner
    }
    // by BS: This is my first attempt to get rid of the hardcoded button bitmask. Note that I do not own any device supported by
    // this backend, so this code is based on guessing. Tested on the LIDE 20, should work for 1-button devices, too.
    ...
    case 5: // only tested for the Genius ColorPage-HR6 V2...
        ...
        if ( ( bytes[ 0 ] & 0x10 ) == 0x10 )
            button += 4;
        if ( ( bytes[ 0 ] & 0x0c ) == 0x0c )
            button += 8;
        if ( ( bytes[ 0 ] & 0x18 ) == 0x18 )
            button += 16;
        break;
    }
    if ( button )
        syslog( LOG_INFO, "scannerbutton: %d", button );
    return button;
}


const char *scanbtnd_get_sane_device_descriptor( scanner_t *scanner ) { return scanner->sane_device; }
...
int main( int argc, char **argv ) {
    int status;
    int button = 0;
    status = scanbtnd_init();
    scanner_t *scanner = (scanner_t *)scanbtnd_get_supported_devices();
    if ( scanner ) {
        status = scanbtnd_open( scanner );
        button = scanbtnd_get_button( scanner );
        printf( "%d\n", button );
        status = scanbtnd_close( scanner );
    }
    scanbtnd_exit();
    return button;
}
