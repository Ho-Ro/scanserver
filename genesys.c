...
static const int supported_usb_devices[ NUM_SUPPORTED_USB_DEVICES ][ 3 ] = {
    ...
    { 0x04a9, 0x190a, 5 }, // CanoScan LiDE 210 (PDF, PDF-FINISH, SCAN, COPY, MAIL)
};
...
const scanner_t *scanbtnd_get_supported_devices( void ) { return genesys_scanners; }
...
int scanbtnd_get_button( scanner_t *scanner ) {
    ...
    num_bytes =
        libusb_control_msg( (libusb_device_t *)scanner->internal_dev_ptr, 0x40, 0x0c, 0x0083, 0x0000, (void *)bytes, 0x0001 );

    if ( num_bytes != 1 ) {
        syslog( LOG_WARNING,
                "genesys-backend: communication error: "
    ...
    // returns 0x1f xored with the keys pressed
    ...
    num_bytes =
        libusb_control_msg( (libusb_device_t *)scanner->internal_dev_ptr, 0xc0, 0x0c, 0x0084, 0x0000, (void *)bytes, 0x0001 );
    if ( num_bytes != 1 ) {
    ...
    // xor with mask and use only lower 5 bit
