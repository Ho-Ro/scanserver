...
libusb_device_t *libusb_get_devices( libusb_handle_t *handle ) { return handle->devices; }
...
int libusb_open( libusb_device_t *device ) {
    ...
    if ( device->handle == NULL ) {
        syslog( LOG_WARNING, "libusbi: could not open device %s", device->location );
        return -ENODEV;
    }
    ...
    case 0:
        return 0;
    case -ENOMEM:
        syslog( LOG_WARNING, "libusbi: could not claim interface for device %s. (ENOMEM)", device->location );
        usb_close( device->handle );
        return -ENODEV;
    case -EBUSY:
        syslog( LOG_WARNING, "libusbi: could not claim interface for device %s. (EBUSY)", device->location );
        usb_close( device->handle );
        return -EBUSY;
    default:
        syslog( LOG_WARNING, "libusbi: could not claim interface for device %s. (code=%d)", device->location, result );
        usb_close( device->handle );
        return -ENODEV;
    }
}
...
int libusb_close( libusb_device_t *device ) {
    ...
    if ( result < 0 ) {
        syslog( LOG_WARNING, "libusbi: could not release interface, error code=%d, device=%s", result, device->location );
        return result;
    }
    result = usb_close( device->handle );
    if ( result < 0 ) {
        syslog( LOG_WARNING, "libusbi: could not close usb device, error code=%d, device=%s", result, device->location );
        return result;
    }
    return 0;
}
